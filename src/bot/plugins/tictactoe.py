...
def disable_all(rows: t.MutableSequence[flare.Row]) -> None:
    for row in rows:
        for component in row:
            assert isinstance(component, TicTacToe)
...
class TicTacToe(flare.Button, label=" "):
    ...
    async def callback(self, ctx: flare.Context) -> None:
        ...
        if ctx.author != (self.player_1 if self.turn else self.player_2):
            await ctx.respond(
                "You can't play! It is not your turn.", flags=hikari.MessageFlag.EPHEMERAL
            )
            return
        ...
        for row in rows:
            for component in row:
                ...
                if component.x == self.x and component.y == self.y:
                    component.set_label("X" if component.turn else "O")
...
@plugin.include
@crescent.command(description="Play tictactoe.")
async def tictactoe(ctx: utils.Context, opponent: hikari.User) -> None:
    def get_tac(x: int, y: int) -> TicTacToe:
        return TicTacToe(x, y, ctx.user, opponent)
